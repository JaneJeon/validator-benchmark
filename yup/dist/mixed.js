...
class RefSet {
  ...
  toArray() {
    return (0, _toArray.default)(this.list).concat((0, _toArray.default)(this.refs.values()));
  }
...
const proto = SchemaType.prototype = {
  ...
  clone() {
    ...
    return (0, _cloneDeepWith.default)(this, value => {
      if ((0, _isSchema.default)(value) && value !== this) return value;
    });
  },
  ...
  _cast(rawValue) {
    let value = rawValue === undefined ? rawValue : this.transforms.reduce((value, fn) => fn.call(this, value, rawValue), rawValue);
  ...
  _validate(_value, options = {}) {
    ...
    return (0, _runValidations.default)({
      ...
      path,
      sync
    }).then(value => (0, _runValidations.default)({
      path,
      sync,
      value,
      endEarly,
  ...
  isValid(value, options) {
    return this.validate(value, options).then(() => true).catch(err => {
  ...
  test(...args) {
    ...
    let isExclusive = opts.exclusive || opts.name && next._exclusive[opts.name] === true;
  ...
  when(keys, options) {
    ...
    var next = this.clone(),
        deps = [].concat(keys).map(key => new _Reference.default(key));
    deps.forEach(dep => {
...
for (const method of ['validate', 'validateSync']) proto[`${method}At`] = function (path, value, options = {}) {
  const {
    parent,
    parentPath,
    schema
  } = (0, _reach.getIn)(this, path, value, options.context);
  return schema[method](parent && parent[parentPath], { ...options,
    parent,
    path
  });
};
